...
% ---------------------------------------------------------------------------
\subsection{OE1: Implementación del módulo PLPUF en HDL}
\label{sec:proc_hdl}
...
% ---------------------------------------------------------------------------
\subsection{OE2: Desarrollo de la interfaz AXI-slave}
\label{sec:proc_axi}
...
% ---------------------------------------------------------------------------
\subsection{OE3: Integración del sistema SoC}
\label{sec:proc_soc}
...
% ---------------------------------------------------------------------------
\subsection{OE4: Protocolo de evaluación estadística}
\label{sec:proc_eval}
...
Se diseñó un experimento automatizado que ejecutó 10~corridas independientes
...
% ---------------------------------------------------------------------------
\subsection{Resultados de la implementación HDL (OE1)}
\label{sec:res_oe1}
...
éxito las minúsculas \textbf{variaciones físicas de los retardos} en los caminos de
...
% ---------------------------------------------------------------------------
\subsection{Pruebas de la interfaz AXI (OE2)}
\label{sec:res_oe2}

Antes de autorizar la evaluación masiva de seguridad, se realizaron pruebas de
...
Las pruebas ejecutadas y aprobadas en hardware incluyeron:
\begin{enumerate}
  \item \textbf{Lectura y escritura de registros de configuración:} Se verificó
    que el procesador pudiera escribir y recuperar sin alteraciones los valores
    en los registros de DURATION y CHAL.
  \item \textbf{Protección de lectura sobre registros de estado:} Se intentaron
    ejecutar escrituras indebidas sobre los registros de RESP (respuesta). La
    prueba comprobó que la interfaz AXI descartó estos datos, garantizando la
    condición de \textit{read-only} del núcleo generador y su inmunidad ante
    modificaciones por software.
  \item \textbf{\textit{Soft reset} e indicadores de estado:} Se constató que
    tras ejecutar una evaluación (donde la bandera \texttt{done} cambia a 1),
    la emisión de un comando de \textit{soft reset} en el registro CTRL
    condujo a una limpieza exitosa de las banderas y la correcta restauración
    de la FSM al estado de reposo, indicando disponibilidad.
\end{enumerate}

Estos resultados con diagnósticos \texttt{ALL PASS} en las 10~corridas certifican
que el diseño de prioridad concurrente del mapa de registros opera sin anomalías,
cumpliendo el OE2 con un canal confiable entre CPU y hardware dedicado.

% ---------------------------------------------------------------------------
\subsection{Prueba de integración del SoC (OE3)}
\label{sec:res_oe3}
...
operacional lograda a lo largo de las 10~corridas independientes evidencia
...
% ---------------------------------------------------------------------------
\subsection{Resultados de la evaluación de seguridad (OE4)}
\label{sec:res_oe4}
...
desviación estándar calculadas sobre las 10~corridas independientes.
...
Un análisis del mapa de estabilidad sobre las 10~corridas indicó un promedio
